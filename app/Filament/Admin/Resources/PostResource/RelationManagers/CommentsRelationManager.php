...
class CommentsRelationManager extends RelationManager
{
    ...
    {
        return __('Коментарі до публікації') . ' ' . \Str::limit($ownerRecord->title, 50);
    }
                ...
                Section::make(__('Коментар'))
                    ->schema([
                        ...
                        Select::make('parent_id')
                            ...
                            ->relationship('parent', 'content', fn ($query) => $query->where('commentable_type', Post::class)->where('commentable_id', $this->ownerRecord->id))
                            ->nullable()
                            ->searchable()
                            ->preload(),
